...
// - Auto-backup: 前回から24時間経過、またはアプリで変更を記録した場合に保存
...
export const BACKUP_TABLES = [
  ...
  'health_reserv',
  'health_data',
  'health_reservation_closed_dates',
];
...
async function fetchAllRows(table, session) {
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    const res = await fetch(
      ...
      { headers: authHeaders(session), signal: fetchTimeout() }
    );
    if (!res.ok) {
...
const AUTO_BACKUP_MIN_AGE_MS = 24 * 60 * 60 * 1000;
const BACKUP_DIRTY_KEY = 'healthcheck_backup_dirty';
...
export async function performBackup(session, { downloadLocal = true, prune = true, skipIfUnchanged = false } = {}) {
  ...
  _backupInFlight = (async () => {
    const dirtyToken = localStorage.getItem(BACKUP_DIRTY_KEY);
    // 変更の記録がなく、直近24時間にバックアップ済みなら全テーブルを読まない。
    if (skipIfUnchanged && !dirtyToken) {
      try {
        const backups = await listStorageBackups(session);
        const latest = backups.reduce((a, b) =>
          !a || getBackupItemTime(b) > getBackupItemTime(a) ? b : a, null);
        const latestTime = latest ? getBackupItemTime(latest) : 0;
        if (latestTime > 0 && Date.now() - latestTime < AUTO_BACKUP_MIN_AGE_MS) {
          setLastBackupTime(latestTime);
          return { skipped: true, fileName: latest.name, payload: null, pruneResult: null };
        }
      } catch {
        // 一覧取得に失敗した場合は従来どおりバックアップを試みる。
      }
    }
    ...
    if (dirtyToken && localStorage.getItem(BACKUP_DIRTY_KEY) === dirtyToken) {
      localStorage.removeItem(BACKUP_DIRTY_KEY);
    }
...
async function upsertRows(table, rows, session) {
  ...
  const BATCH = 500;
  const conflictKey = table === 'health_reservation_closed_dates' ? 'date' : 'id';
