...
// - Upload: Supabase Storage bucket "backups" under "healthcheck/" folder
...
const FILE_PREFIX = 'healthcheck_backup';
...
// Order matters for restore (parent first if FK exists)
...
function makeFileName(date = new Date()) {
  ...
  return `${FOLDER}/${FILE_PREFIX}_${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}_${pad(date.getHours())}-${pad(date.getMinutes())}-${pad(date.getSeconds())}.json`;
}
...
function parseBackupItemTime(item) {
  const times = [item?.updated_at, item?.created_at, item?.last_accessed_at]
    .map((value) => value ? Date.parse(value) : 0)
    .filter((value) => Number.isFinite(value) && value > 0);

  const match = item?.name?.match(/healthcheck_backup_(\d{4})-(\d{2})-(\d{2})_(\d{2})-(\d{2})-(\d{2})\.json$/);
  if (match) {
    const [, y, mo, d, h, mi, s] = match;
    times.push(new Date(Number(y), Number(mo) - 1, Number(d), Number(h), Number(mi), Number(s)).getTime());
  }
...
export async function getLatestStorageBackupTime(session) {
  const items = await listStorageBackups(session);
  const times = items.map(parseBackupItemTime).filter((value) => value > 0);
  return times.length ? Math.max(...times) : 0;
}
...
export async function shouldRunAutoBackup(session) {
  ...
  let last = 0;
  try {
    last = await getLatestStorageBackupTime(session);
    if (last) setLastBackupTime(last);
  } catch (err) {
    console.warn('Storage backup time check failed; falling back to local backup time.', err);
  }

  if (!last) last = getLastBackupTime();
  if (!last) return true;
