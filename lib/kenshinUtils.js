...
// 旧形式（万/μL、例: 480）を現行形式（×10⁶/μL、例: 4.80）へ変換する。
export const normalizeRbcToMillions = (value) => {
  const normalized = String(value ?? '').normalize('NFKC').replace(/,/g, '').trim();
  if (!normalized) return '';
  const number = Number(normalized);
  if (!Number.isFinite(number)) return normalized;
  return Math.abs(number) >= 100 ? (number / 100).toFixed(2) : normalized;
};

export const calculateAgRatio = (totalProtein, albumin) => {
  ...
  const total = normalizeNumber(totalProtein);
  const alb = normalizeNumber(albumin);
  if (total == null || alb == null || total <= alb) return '';
  return (alb / (total - alb)).toFixed(1);
};

// 採血基準値テーブル
export const BLOOD_REFERENCE_RANGES = {
  wbc:          { min: 3.3,   max: 8.6 },
  rbc:          { M: { min: 4.35, max: 5.55 }, F: { min: 3.86, max: 4.92 } },
  hemoglobin:   { M: { min: 13.7, max: 16.8 }, F: { min: 11.6, max: 14.8 } },
...
export const getBloodArrow = (name, value, gender) => {
  ...
  const comparableValue = name === 'rbc' ? normalizeRbcToMillions(value) : value;
  const num = parseFloat(comparableValue);
  if (isNaN(num)) return '';
...
export const kenshinInitialState = {
  ...
  // 腫瘍マーカー
  cea: '', ca199: '', psaValue: '', ca125: '', ca153: '', afp: '',
  // その他採血項目
  bnp: '', hbsAg: '', hbsAb: '', hcvAb: '', syphilisSTS: '', mrsaStaph: '',
  // その他検査項目
